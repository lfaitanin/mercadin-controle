...
export default function Register({ onAuth }: { onAuth: (t: string) => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-2 py-6">
      <form onSubmit={handleRegister} className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm space-y-6 relative">
        ...
        <h2 className="text-2xl font-bold text-primary mb-2 text-center">Criar conta</h2>
        ...
        <button
          ...
          disabled={loading}
        >{loading ? (<><Spinner className="inline-block align-middle mr-2" />Registrando...</>) : 'Registrar'}</button>
      </form>
    </div>
  )
} 